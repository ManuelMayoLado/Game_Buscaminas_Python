...
def num(pos):
	return int(pos[0]+pos[1]*NUM_CADRADOS_FILA)
	
def pos(num):
	return [num % NUM_CADRADOS_FILA, int(num / NUM_CADRADOS_FILA)]
...
while ON:
	...
	if (pos_mouse[0] > MARCO and pos_mouse[1] > MARCO and pos_mouse[0] < ANCHO_VENTANA-MARCO and pos_mouse[1] < ALTO_VENTANA-MARCO 
			...
			and not GAME_OVER):
		pos_casilla_mouse = [int((pos_mouse[0]-MARCO)/LADO_CADRO),int((pos_mouse[1]-MARCO)/LADO_CADRO)]
	else:
		pos_casilla_mouse = False
		
	if pos_casilla_mouse:
		num_casilla = int(num(pos_casilla_mouse))
